    ...
    for (let i = 0; i < inputTexts.length; i++) {
      try {
        const response = await axios.post(
          'https://xdwvg9no7pefghrn.us-east-1.aws.endpoints.huggingface.cloud', // Replace with your actual API endpoint
          { inputs: inputTexts[i] },
          {
            headers: {
              Accept: 'image/png',
  ...
  return (
    <div className="comic-panel-generator">
      ...
      <div className="input-container">
        {inputTexts.slice(0, 5).map((text, index) => (
        ...
        {inputTexts.slice(5).map((text, index) => (
          <div key={index + 5} className="input-wrapper">
            <input
              type="text"
              value={text}
              placeholder={`Enter text for panel ${index + 6}`}
